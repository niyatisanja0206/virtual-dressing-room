...
argp.add_argument('-i',
                  ...
                  default='D:/computervision/virtual dressing room/VDR/CIHP_PGN/datasets/CIHP/images')
argp.add_argument('-o',
                  '--output',
                  type=str, help='Path of the input dir',
                  default='D:/computervision/virtual dressing room/VDR/CIHP_PGN/datasets/CIHP/output')
...
RESTORE_FROM = 'D:\computervision\virtual dressing room\VDR\checkpoints\CIHP_pgn'

def remove_false_tshirt_patches(mask, tshirt_label=5, lower_label=9):
    """
    Removes small or mislocated t-shirt patches from the segmentation mask.
    """
    # Make a copy
    clean_mask = mask.copy()

    # Extract t-shirt region
    tshirt_mask = (mask == tshirt_label).astype(np.uint8)

    # Find connected components in the t-shirt mask
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(tshirt_mask, connectivity=8)

    # Estimate height threshold (e.g., above 2/3 of image is allowed)
    h = mask.shape[0]
    allowed_y = h * 0.6
    ...
    for i in range(1, num_labels):  # Skip background
        ...
        # Remove component if it's too low (i.e., close to pants) or too small
        if cy > allowed_y or area < 500:  # adjust threshold as needed
            clean_mask[labels == i] = lower_label  # e.g., 9 for pants
...
def main():
    ...
    for step in range(NUM_STEPS):
        ...
        cv2.imwrite('{}/{}_mask.png'.format(parsing_dir, img_id), parsing_[0,:,:,0])
        # sio.savemat('{}/{}.mat'.format(parsing_dir, img_id), {'data': scores[0,:,:]})
        
        cv2.imwrite('{}/{}.png'.format(edge_dir, img_id), edge_[0,:,:,0] * 255)
        clean_parsing = remove_false_tshirt_patches(parsing_[0, :, :, 0])
        cv2.imwrite('{}/{}_mask_cleaned.png'.format(parsing_dir, img_id), clean_parsing)
